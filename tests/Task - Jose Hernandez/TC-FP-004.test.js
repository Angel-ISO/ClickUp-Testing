...
taggedDescribe(
  ...
  "TC-FP-004 - Verify that two or more related tasks can be merged into a single task",
  () => {
    let folderId;
    let listId;
    let createdTaskIds = [];
    ...
    afterEach(async () => {
      for (const taskId of createdTaskIds) {
        await tasksService.deleteTask(taskId).catch((error) => {
    ...
    it("Merge Tasks - Combine Three Tasks into One", async () => {
      ...
      const mergeResponse = await tasksService.mergeTasks(
        mainTaskId,
        mergeData
      );
      ...
      console.log(
        `Tasks merged successfully: ${task2Id} and ${task3Id} into ${mainTaskId}`
      );
      ...
      console.log(
        `Merge verified - Description contains all three task descriptions`
      );
    });
    ...
    it("Merge Tasks - Verify Source Tasks Are Deleted", async () => {
      ...
      const mainTaskResponse = await tasksService.createTask(
        listId,
        mainTaskData
      );
      ...
      const sourceTaskResponse = await tasksService.createTask(
        listId,
        sourceTaskData
      );
      ...
      await tasksService.mergeTasks(mainTaskId, mergeData);

      const sourceTaskGetResult = await tasksService
        .getTask(sourceTaskId)
        .catch((error) => error);

      if (sourceTaskGetResult && sourceTaskGetResult.response?.status === 404) {
        console.log(
          `Source task ${sourceTaskId} was successfully deleted after merge`
        );
      } else {
        throw new Error(
          `Source task ${sourceTaskId} should have been deleted but still exists`
        );
      }
