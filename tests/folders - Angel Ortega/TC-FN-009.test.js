...
taggedDescribe(
  buildTags({
    smoke: true,
    funcionalidad: FUNCIONALIDADES.FOLDERS,
    negative: true,
  }),
  ...
  () => {
    ...
    it("Get Folder - Non-existent Folder ID", async () => {
      ...
      const getResult = await foldersService.getFolderResult(
        nonExistentFolderId
      );
      ...
      console.log(
        `Correctly returned error for non-existent folder: ${nonExistentFolderId}`
      );
    });
    ...
    it("Update Folder - Non-existent Folder ID", async () => {
      ...
      const updateResult = await foldersService.updateFolderResult(
        nonExistentFolderId,
        {
          name: "Updated Name",
        }
      ...
      console.log(
        `Correctly returned error for updating non-existent folder: ${nonExistentFolderId}`
      );
    });
    ...
    it("Delete Folder - Non-existent Folder ID", async () => {
      ...
      console.log(
        `Correctly returned error for deleting non-existent folder: ${nonExistentFolderId}`
      );
    });
  }
);
